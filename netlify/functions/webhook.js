...
var TL = {
  "prima-ps5": "PrimÃÂ¡ria PS5",
  "prima-ps4": "PrimÃÂ¡ria PS4",
  "secun-ps5": "SecundÃÂ¡ria PS5",
  "secun-ps4": "SecundÃÂ¡ria PS4"
};

var ALT = "ATENÃÂÃÂO - NÃÂO CRIAR O USUÃÂRIO COMO CONVIDADO ! - APERTE EM INICIAR SESSÃÂO MANUALMENTE";

var TP5 = [
  "ParabÃÂ©ns! VocÃÂª adquiriu a sua licenÃÂ§a PRIMÃÂRIA que te dÃÂ¡ direito a usar esta conta em 1 VÃÂDEO GAME!",
  "",
...
var TS5 = TP5.replace("PRIMÃÂRIA", "SECUNDÃÂRIA").replace("UsuÃÂ¡rio primÃÂ¡rio", "UsuÃÂ¡rio secundÃÂ¡rio");
var TS4 = TP4.replace("PRIMÃÂRIA", "SECUNDÃÂRIA").replace("UsuÃÂ¡rio primÃÂ¡rio", "UsuÃÂ¡rio secundÃÂ¡rio");
...
function pLic(name) {
  return (name || "").toLowerCase().includes("secund") ? "SecundÃÂ¡ria" : "PrimÃÂ¡ria";
}
...
async function makeDraft(tok, item, order, cn, num, sn, wpp, cm) {
  ...
  var lm = (item.meta_data || []).find(function(m) {
    return m.key === "LicenÃÂ§as" || m.key === "pa_licencas" || (m.key || "").toLowerCase().includes("licen");
  });
  var licRaw = lm ? (lm.display_value || lm.value || "") : "";
  var lic;
  if (licRaw) {
    var licL = licRaw.toLowerCase();
    if (licL.includes("secund")) lic = "SecundÃÂ¡ria";
    else if (licL.includes("prim")) lic = "PrimÃÂ¡ria";
    else lic = licRaw;
  } else {
    lic = pLic(item.name);
  }
  ...
  var cr = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      system: "Agente da loja " + sn + ". Escreva: [S]saudaÃÂ§ÃÂ£o 1 linha[/S][E]encerramento 1 linha[/E]",
  ...
  var sau = sm ? sm[1].trim() : "OlÃÂ¡, " + cn + "!";
  var enc = em ? em[1].trim() : "Qualquer dÃÂºvida estamos ÃÂ  disposiÃÂ§ÃÂ£o!";
  var wl = wpp ? "WhatsApp: " + wpp + "<br>" : "";
  var arow = '<p style="color:red;font-weight:bold;margin:6px 0">' + ALT + '</p>';
  arow = arow + arow + arow;
  var tlink = tut.link
    ? '<p><strong>VÃÂDEO DE TUTORIAL (' + TL[tk] + '): </strong><a href="' + tut.link + '">' + tut.link + '</a></p>'
    : "";
  var html = '<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body style="font-family:Arial,sans-serif;font-size:14px;line-height:1.7;color:#222">'
    ...
    + '<tr><td style="padding:8px 12px;border:1px solid #ccc;font-weight:bold">CÃÂ³digo</td><td style="padding:8px 12px;border:1px solid #ccc;background:#fff8e1"></td></tr>'
    + '</table>'
    + arow + tlink
    + '<p>' + nl2br(tut.text) + '</p>'
    + '<p>' + enc + '</p>'
